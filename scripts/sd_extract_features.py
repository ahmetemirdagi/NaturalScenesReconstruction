...
class batch_generator_external_images(Dataset):
    def __init__(self, data_path):
        ...
        self.is_npy = data_path.endswith('.npy')
        
        if self.is_npy:
            # Load .npy file
            self.im = np.load(data_path).astype(np.uint8)
            print(f"Loaded .npy data shape: {self.im.shape}")
        else:
...
# For test data, use PNG files from VDVAE results
image_path = 'results/vdvae/subj01'  # Directory containing PNG files
...
np.save(f'{output_dir}/nsd_sd_train.npy', test_features)
# For training data, use original .npy file
